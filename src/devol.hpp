...
#include "permute.hpp"      // declarations for both Rcpp and RcppArmadillo offering Armadillo classes

// #include <google/profiler.h>

void devol(double VTR, double f_weight, double f_cross, int i_bs_flag,
           const arma::colvec & fa_minbound, const arma::colvec & fa_maxbound,
           std::function<double(const Rcpp::NumericVector &,const arma::colvec &,const arma::colvec &)> eval,                                       // function to be optimized, either R or C++
           const arma::colvec & xr, const arma::colvec & yr, int i_trace,
  ...
  while ((i_iter < i_itermax) && (t_bestC > VTR) && (i_iter_tol <= i_steptol)) {    // main loop ====================================
    ...
    for (int i = 0; i < i_NP; i++) {        // ----start of loop through ensemble------------------------
      ...
      double t_tmpC = eval(par, xr, yr);                              // Evaluate mutant in t_tmpP
